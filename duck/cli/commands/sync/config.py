...
class DuckSyncConfig:
    ...
    Args:
        ...
        native:
            Whether native configuration is present in duck.toml.
    """
...
def find_config(start: Path | None = None) -> Path:
    ...
    Args:
        start:
            Directory to begin searching from. Defaults to the cwd.

    Returns:
        Path to the discovered duck.toml.

    Raises:
        ConfigNotFoundError:
            If no duck.toml is found up to the filesystem root.
    """
...
def load_config(path: Path | None = None) -> DuckSyncConfig:
    ...
    Args:
        path:
            Explicit path to a duck.toml file. Auto discovered when omitted.

    Returns:
        The parsed DuckSyncConfig.
...
def parse_dependency_group(raw: dict) -> DependencyGroup:
    ...
    Args:
        raw:
            The raw dict for that TOML table.
...
def merge_dependencies(
    ...
    """
    Merges dependency lists while preserving their original order.

    Args:
        existing:
            Dependencies already declared by the user.

        additional:
            Dependencies automatically supplied by another configuration.

    Returns:
        Combined dependency list with duplicates removed.
    """
