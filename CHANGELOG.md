...
- You can now declaratively bind events to HTML components using the `event_handlers` argument (e.g. `Button(event_handlers=[{"click": on_button_click, **extra_kwargs}])`); for document-level events such as `DOMContentLoaded`, use the `document_event_handlers` argument on `Page` with the same event configuration format.
