...
* - {py:obj}`get_file_upload_handler <duck.settings.loaded.get_file_upload_handler>`
  - ```{autodocx-docstring} duck.settings.loaded.get_file_upload_handler
    :summary:
    ```
* - {py:obj}`get_jwt_store <duck.settings.loaded.get_jwt_store>`
  - ```{autodocx-docstring} duck.settings.loaded.get_jwt_store
    :summary:
    ```
...
````{py:function} get_jwt_store()
:canonical: duck.settings.loaded.get_jwt_store
...
````{py:function} get_session_storage() -> type
