...
```{autodocx-docstring} duck.http.middlewares.security.csrf.CSRFMiddleware.get_error_response
```
...
```{autodocx-docstring} duck.http.middlewares.security.csrf.CSRFMiddleware.process_request
```
...
```{autodocx-docstring} duck.http.middlewares.security.csrf.CSRFMiddleware.process_response
```
