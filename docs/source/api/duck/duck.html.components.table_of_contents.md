...
* - {py:obj}`BASE_STYLE <duck.html.components.table_of_contents.BASE_STYLE>`
  - ```{autodocx-docstring} duck.html.components.table_of_contents.BASE_STYLE
    :summary:
    ```
````
...
````{py:data} BASE_STYLE
:canonical: duck.html.components.table_of_contents.BASE_STYLE
