...
````{py:method} get_session_key_from_cookie(request: duck.http.request.HttpRequest) -> typing.Optional[str]
:canonical: duck.http.middlewares.contrib.session.SessionMiddleware.get_session_key_from_cookie
:classmethod:

```{autodocx-docstring} duck.http.middlewares.contrib.session.SessionMiddleware.get_session_key_from_cookie
```
...
```{autodocx-docstring} duck.http.middlewares.contrib.session.SessionMiddleware.process_response
```
